...
#import "DENotificationCenterDetailPane.h"
...
#define EVENT_SOUNDS_ALERT_SHORT	AILocalizedString(@"Display a notification",nil)
#define EVENT_SOUNDS_ALERT_LONG		AILocalizedString(@"Display a notification",nil)

#define NOTIFICATION_CENTER_IDENTIFIER		@"NotificationCenter"
...
- (void)installPlugin
{
	...
	[adium.contactAlertsController registerActionID:NOTIFICATION_CENTER_IDENTIFIER withHandler:self];
}
...
{
	return [DENotificationCenterDetailPane actionDetailsPane];
}
...
- (BOOL)performActionID:(NSString *)actionID forListObject:(AIListObject *)listObject withDetails:(NSDictionary *)details triggeringEventID:(NSString *)eventID userInfo:(id)userInfo
{
    ...
    NSLog(@"Subtitle: %@", notice.subtitle);
    NSLog(@"Info: %@", notice.informativeText);
